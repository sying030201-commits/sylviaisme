      ...
      <AnimatePresence mode="wait">
        {mode === 'THEN' ? (
          ...
          >
            ...
            {/* Centered Toggle Inside Card - Enhanced Visibility & Guidance */}
            <div className="flex flex-col items-center mb-12 relative">
              <div className="absolute -inset-4 bg-gradient-to-r from-then/30 via-now/30 to-then/30 blur-2xl opacity-60 group-hover/card:opacity-100 transition-opacity duration-1000" />
              <div className="relative flex items-center bg-bg-deep/90 backdrop-blur-2xl p-2.5 rounded-[28px] border-2 border-white/20 shadow-[0_0_50px_rgba(0,0,0,0.5)] group/toggle scale-110 md:scale-125">
                {/* Animated Scan Line */}
                <motion.div 
                ...
                <motion.div 
                  ...
                  className={`absolute inset-y-2 rounded-[22px] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${mode === 'THEN' ? 'left-2 w-[calc(50%-8px)] bg-then shadow-[0_0_30px_rgba(74,85,104,0.7)]' : 'left-[calc(50%+2px)] w-[calc(50%-8px)] bg-now shadow-[0_0_30px_rgba(196,167,125,0.7)]'}`}
                />
                <button 
                  onClick={() => setMode('THEN')}
                  className={`relative z-10 px-12 py-5 rounded-[22px] text-[12px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 ${mode === 'THEN' ? 'text-white scale-105' : 'text-text-muted hover:text-white'}`}
                >
                  <Clock size={18} className={mode === 'THEN' ? 'animate-pulse' : ''} /> 过去：手动时代
                </button>
                <button 
                  onClick={() => setMode('NOW')}
                  className={`relative z-10 px-12 py-5 rounded-[22px] text-[12px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 ${mode === 'NOW' ? 'text-bg-deep scale-105' : 'text-text-muted hover:text-white'}`}
                >
                  <Zap size={18} className={mode === 'NOW' ? 'animate-pulse' : ''} /> 现在：AI 进化
                </button>
              </div>
              ...
              <div className="mt-8 flex flex-col items-center gap-3">
                <div className="flex items-center gap-6">
                  <motion.div 
                    ...
                    className="h-px w-12 bg-gradient-to-r from-transparent to-now" 
                  />
                  <motion.p 
                    animate={{ opacity: [0.6, 1, 0.6], scale: [1, 1.1, 1] }}
                    transition={{ repeat: Infinity, duration: 2 }}
                    className="text-[12px] font-black text-now uppercase tracking-[0.5em] flex items-center gap-3 bg-now/10 px-4 py-1 rounded-full border border-now/20 shadow-[0_0_15px_rgba(196,167,125,0.2)]"
                  >
                    <ArrowLeftRight size={14} className="text-now" />
                    点击此处 见证进化
                  </motion.p>
                  <motion.div 
                    animate={{ x: [10, 0, 10] }}
                    transition={{ repeat: Infinity, duration: 1.5 }}
                    className="h-px w-12 bg-gradient-to-l from-transparent to-now" 
                  />
                </div>
              </div>
            </div>
                  ...
                  <>
                    ...
                    <div className="rounded-2xl overflow-hidden aspect-video relative group/img border border-white/10 shadow-2xl">
                        <img 
                          ...
                          className={`w-full h-full object-cover transition-all duration-1000 ${work.id === 'kol-audit' || work.id === 'comp-monitor' ? 'opacity-100 group-hover/img:scale-105' : 'opacity-100 group-hover/img:scale-110'}`}
                          referrerPolicy="no-referrer"
                        />
    ...
    <section id="work" className="py-32 bg-bg-charcoal/50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="mb-24 flex flex-col items-center text-center">
          ...
          {/* Global Evolution Controller - Enhanced Prominence */}
          <div className="relative group/global-toggle mt-8">
            {/* Discovery Hint */}
            <motion.div 
              initial={{ opacity: 0, y: 10 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="px-6 py-3 bg-now text-bg-deep text-[12px] font-black rounded-full shadow-[0_0_30px_rgba(196,167,125,0.5)] flex items-center gap-2 animate-bounce border-2 border-white/20">
                <Sparkles size={14} /> 点击此处，见证全域进化
              </div>
            </motion.div>

            <div className="absolute -inset-6 bg-gradient-to-r from-then/40 via-now/40 to-then/40 blur-3xl opacity-0 group-hover/global-toggle:opacity-100 transition-opacity duration-700" />
            <div className="relative flex items-center bg-bg-deep p-3 rounded-[40px] border-2 border-white/20 shadow-[0_0_60px_rgba(0,0,0,0.6)] scale-110 md:scale-125">
              <motion.div 
                animate={{ 
                  left: globalMode === 'THEN' ? '12px' : 'calc(50% + 2px)',
                  width: 'calc(50% - 14px)'
                }}
                transition={{ type: 'spring', damping: 25, stiffness: 120 }}
                className={`absolute inset-y-2 rounded-[30px] ${globalMode === 'THEN' ? 'bg-then shadow-[0_0_30px_rgba(74,85,104,0.6)]' : 'bg-now shadow-[0_0_30px_rgba(196,167,125,0.6)]'}`}
              />
              <button 
                onClick={() => setGlobalMode('THEN')}
                className={`relative z-10 px-14 py-6 rounded-[30px] text-base font-black tracking-widest transition-all duration-500 flex items-center gap-4 ${globalMode === 'THEN' ? 'text-white' : 'text-text-muted hover:text-white'}`}
              >
                <Clock size={24} className={globalMode === 'THEN' ? 'animate-spin-slow' : ''} /> 全局过去
              </button>
              <button 
                onClick={() => setGlobalMode('NOW')}
                className={`relative z-10 px-14 py-6 rounded-[30px] text-base font-black tracking-widest transition-all duration-500 flex items-center gap-4 ${globalMode === 'NOW' ? 'text-bg-deep' : 'text-text-muted hover:text-white'}`}
              >
                <Zap size={24} className={globalMode === 'NOW' ? 'animate-pulse' : ''} /> 全局现在
              </button>
            </div>
            <p className="mt-10 text-[12px] font-black text-now uppercase tracking-[0.5em] opacity-60 bg-now/5 px-6 py-2 rounded-full border border-now/10 inline-block">
              一键切换 见证全域进化
            </p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-xs font-bold truncate max-w-full">{copied === 'email' ? '已复制' : 'im.sylvia@outlook.com'}</span>
                  </div>
                </MagneticButton>
                <MagneticButton 
                  onClick={() => copyToClipboard('19533296523', 'phone')}
                  className="p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-colors relative group"
                >
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-xs font-bold">{copied === 'phone' ? '已复制' : '19533296523'}</span>
                  </div>
                </MagneticButton>
              </div>
